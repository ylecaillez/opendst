 ...
 * Specifications.p
 *
 * P spec monitors that validate instrumented code follows the Java Socket API
 * contract. These monitors are compiled to Java via PObserve and check runtime
 * traces emitted by socket decorators (TracingSocket, TracingServerSocket).
 *
 * Safety properties (violations cause immediate assertion failures):
 *   1. DataIntegrity: data read must be a prefix of data written by the peer.
 *   2. NoWriteAfterClose: no successful write after close().
 *   3. NoPhantomData: no data read from unconnected sockets.
 *   4. IOExceptionOnClosedSocket: no successful I/O on closed sockets.
 *   5. NoWriteAfterShutdownOutput: no successful write after shutdownOutput().
 *   6. NoReadAfterShutdownInput: no data read after shutdownInput().
 *   7. AvailableConsistency: available() <= unread bytes from peer.
 *
 * Liveness properties (checked at test end — hot state = violation):
 *   8. GracefulShutdownIntegrity: graceful shutdown data fully delivered.
 *   9. DeliveryLiveness: written data eventually read or connection disrupted.
 *  10. EOFLiveness: shutdownOutput EOF eventually read or connection disrupted.
 ******************************************************************************/
 ...
 * be read by the peer before EOF. The writer-side SocketClosed does NOT
 * excuse the delivery obligation — the sender chose graceful shutdown, so
 * the data must arrive. Only the peer (reader) closing or a connection
 * reset excuses the obligation.
 *
...
spec GracefulShutdownIntegrity observes eSpec_ConnectionEstablished, eSpec_DataWritten, eSpec_DataRead, eSpec_ShutdownOutputCompleted, eSpec_EOFRead, eSpec_SocketClosed, eSpec_ConnectionReset, eSpec_IOExceptionRaised
{
    ...
    // Sockets whose graceful obligation has been excused
    ...
    start state NoObligation {
        ...
        on eSpec_SocketClosed do (payload: (socket: machine)) {
            ...
            ExcusePeerObligation(payload.socket);
        }
    ...
    // Excuse the peer's obligation to this socket: if socket S closes,
    // S's peer (the writer) no longer needs to deliver data to S.
    // S's own obligation to deliver data to its peer is NOT excused.
    fun ExcusePeerObligation(socket: machine) {
        var writer: machine;
        if (socket in peerOf) {
 ...
 * unless the connection is disrupted.
 *
 * Delivery is measured at the application level: the monitor tracks
 * eSpec_DataWritten (write() calls) against eSpec_DataRead (read() calls).
 * For the liveness obligation to be fulfilled, the peer must actually call
 * read() and consume the data — buffered data that was never read counts as
 * undelivered.
 *
 * The test must drain remaining sockets (read until EOF or IOException) before
 * emitting TestCompleted, so that the monitor can observe fulfilled delivery
 * obligations on non-excused sockets.
 *
 * Excusal semantics:
 *   - close():  excuses only the peer's obligation to this socket. If socket
 *               S closes, S's peer no longer needs to deliver data to S.
 *               But S's own delivery obligation to its peer is NOT excused —
 *               the writer closing does not forgive undelivered data.
 *   - reset:    excuses both sides (involuntary disruption).
 *
 * Uses hot/cold states: when there are unread bytes pending delivery, the
 * monitor enters a hot state. A liveness violation is flagged if execution
 * ends in a hot state.
 *
...
spec DeliveryLiveness observes eSpec_ConnectionEstablished, eSpec_DataWritten, eSpec_DataRead, eSpec_SocketClosed, eSpec_ConnectionReset, eSpec_IOExceptionRaised
{
    ...
    // Excuse the peer's delivery obligation to this socket: if socket S
    // closes, S's peer no longer needs to deliver written data to S.
    // S's own obligation to deliver data to its peer is NOT excused.
    fun ExcusePeerObligation(socket: machine) {
        var writer: machine;
        if (socket in peerOf) {
 ...
 * EOF, unless the connection is disrupted.
 *
 * Excusal semantics:
 *   - close():  excuses only the peer's EOF obligation to this socket. If
 *               socket S closes, S's peer no longer needs to deliver EOF
 *               to S. But S's own EOF obligation to its peer is NOT
 *               excused — the writer closing does not forgive undelivered EOF.
 *   - reset:    excuses both sides (involuntary disruption).
 *
...
spec EOFLiveness observes eSpec_ConnectionEstablished, eSpec_ShutdownOutputCompleted, eSpec_EOFRead, eSpec_SocketClosed, eSpec_ConnectionReset, eSpec_IOExceptionRaised
{
    ...
    // Excuse the peer's EOF obligation to this socket: if socket S closes,
    // S's peer no longer needs to deliver EOF to S.
    // S's own EOF obligation to its peer is NOT excused.
    fun ExcusePeerObligation(socket: machine) {
        var writer: machine;
        if (socket in peerOf) {
            writer = peerOf[socket];
